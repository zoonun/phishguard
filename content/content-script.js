...
(function () {
  ...
  const AlertBanner = {
    ...
    _buildHTML(totalRisk, riskLevel, results) {
      const cfg = {
        danger: {
          icon: "\u26A0\uFE0F",
          prefix: "\uACBD\uACE0:",
          msg: "\uC774 \uC0AC\uC774\uD2B8\uB294 \uD53C\uC2F1 \uC0AC\uC774\uD2B8\uB85C \uC758\uC2EC\uB429\uB2C8\uB2E4! \uAC1C\uC778\uC815\uBCF4\uB97C \uC785\uB825\uD558\uC9C0 \uB9C8\uC138\uC694.",
        },
        warning: {
          icon: "\u26A1",
          prefix: "\uC8FC\uC758:",
          msg: "\uC774 \uC0AC\uC774\uD2B8\uC5D0\uC11C \uC758\uC2EC\uC2A4\uB7EC\uC6B4 \uC694\uC18C\uAC00 \uAC10\uC9C0\uB418\uC5C8\uC2B5\uB2C8\uB2E4.",
        },
        safe: {
          icon: "\u2705",
          prefix: "\uCC38\uACE0:",
          msg: "\uC77C\uBD80 \uC8FC\uC758 \uC0AC\uD56D\uC774 \uAC10\uC9C0\uB418\uC5C8\uC2B5\uB2C8\uB2E4.",
        },
      }[riskLevel] || {
        icon: "\u2139\uFE0F",
        prefix: "\uC815\uBCF4:",
      ...
      const details = (results || [])
        .filter(
          (r) =>
            r.confidence > 0 ||
            (r.name === "LLMAnalyzer" && r.details && r.details.apiError),
        )
        .sort((a, b) => (b.name === "LLMAnalyzer") - (a.name === "LLMAnalyzer"))
        .map((r) => {
          const cls =
            r.risk >= 70 ? "danger" : r.risk >= 40 ? "warning" : "safe";
          const label = moduleLabels[r.name] || r.name;
          const reason = this._escapeHtml(r.reason || "");

          // 추가 정보 생성 (ContentAnalyzer findings, LLM error)
          let extraHtml = "";

          if (
            r.name === "ContentAnalyzer" &&
            r.details &&
            r.details.findings &&
            r.details.findings.length > 0
          ) {
            const items = r.details.findings
              .map((f) => {
                const bullets =
                ...
                const bulletHtml =
                  bullets.length > 0
                    ? '<ul class="pg-detail__bullets">' +
                      bullets
                        .map((b) => `<li>${this._escapeHtml(b)}</li>`)
                        .join("") +
                      "</ul>"
                    : "";
                return `<li><strong>${this._escapeHtml(f.description)}</strong>${bulletHtml}</li>`;
              })
              .join("");
            extraHtml = `<ul class="pg-detail__findings">${items}</ul>`;
          }
          ...
          if (
            ...
            extraHtml = `<div class="pg-detail__error">
              <span class="pg-detail__error-label">API Response</span>
              <pre class="pg-detail__error-body">${this._escapeHtml(r.details.error)}</pre>
            </div>`;
          }

          const hasExtra = extraHtml.length > 0;
          const itemClass =
            "pg-detail__item" + (hasExtra ? " pg-detail__item--has-extra" : "");
          const chevron = hasExtra
            ? '<span class="pg-detail__chevron">&#9660;</span>'
            : "";
          const extraSection = hasExtra
            ? `<div class="pg-detail__extra">${extraHtml}</div>`
            : "";
          ...
          return `<div class="${itemClass}">
            <div class="pg-detail__summary">
              ...
              <span class="pg-detail__reason">${reason}</span>
              ${chevron}
            </div>
            ${extraSection}
          </div>`;
    ...
    _bindEvents() {
      ...
      // Accordion toggle for items with extra details
      const expandableItems = this._shadowRoot.querySelectorAll(
        ".pg-detail__item--has-extra .pg-detail__summary",
      );
      expandableItems.forEach((summary) => {
        summary.addEventListener("click", () => {
          summary.parentElement.classList.toggle("pg-detail__item--expanded");
        });
      });
    },
...
.pg-detail__item { padding: 8px 0; border-bottom: 1px solid rgba(255,255,255,0.08); }
.pg-detail__item:last-child { border-bottom: none; }
.pg-detail__summary { display: flex; align-items: flex-start; gap: 10px; }
.pg-detail__item--has-extra .pg-detail__summary { cursor: pointer; user-select: none; }
.pg-detail__item--has-extra .pg-detail__summary:hover { opacity: 0.85; }
...
.pg-detail__chevron { font-size: 10px; color: #5a5a7a; flex-shrink: 0; transition: transform 0.25s ease; margin-top: 4px; }
.pg-detail__item--expanded .pg-detail__chevron { transform: rotate(180deg); }
/* Expandable extra details */
.pg-detail__extra { max-height: 0; overflow: hidden; transition: max-height 0.3s ease; margin-left: 120px; }
.pg-detail__item--expanded .pg-detail__extra { max-height: 600px; padding-top: 8px; }
.pg-detail__findings { list-style: none; padding: 0; }
.pg-detail__findings > li { font-size: 12px; color: #c0c0d8; padding: 5px 0; border-bottom: 1px solid rgba(255,255,255,0.05); }
.pg-detail__findings > li:last-child { border-bottom: none; }
.pg-detail__findings > li strong { color: #d0d0e8; font-weight: 600; }
.pg-detail__bullets { list-style: none; padding: 4px 0 0; }
.pg-detail__bullets li { position: relative; padding-left: 14px; font-size: 11px; color: #9999bb; line-height: 1.7; }
.pg-detail__bullets li::before { content: '\u00B7'; position: absolute; left: 4px; color: #a78bfa; font-weight: 700; }
.pg-detail__error { padding-top: 4px; }
.pg-detail__error-label { display: block; font-size: 10px; font-weight: 700; color: #f87171; text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 6px; }
.pg-detail__error-body { background: #16162b; border: 1px solid rgba(248,113,113,0.2); border-radius: 6px; padding: 10px 12px; font-size: 11px; font-family: 'SF Mono','Fira Code','Consolas',monospace; color: #e0e0e0; white-space: pre-wrap; word-break: break-all; line-height: 1.5; margin: 0; max-height: 150px; overflow-y: auto; }
