...
const LLMAnalyzer = {
  ...
  async analyze(context, options = {}) {
    ...
    if (previousResults) {
      ...
      if (avgRisk < 20) {
        ...
        return {
          ...
          confidence: 0,
          reason:
            "다른 감지 모듈에서 안전하다고 판단하여 LLM 분석을 생략합니다.",
    ...
    try {
      ...
      this._recordRequest();
      const systemPrompt =
        prompts?.getSystemPrompt?.() || this._getDefaultSystemPrompt();
