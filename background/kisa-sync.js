...
const KISA_API_URL = 'https://api.odcloud.kr/api/15109780/v1/uddi:707478dd-938f-4155-badb-fae6202ee7ed';
...
const KisaSync = {
  ...
  async startFullSync(serviceKey) {
    ...
    try {
      // 동기화 상태 저장
      await this._setSyncState({ status: 'syncing', currentPage: 0, startedAt: Date.now() });
      ...
      Logger.info(`[KisaSync] Total: ${totalCount} entries, ${totalPages} pages`);

      const allHostnames = new Set();

      // 첫 페이지 데이터 처리
      this._extractHostnames(firstPage.data, allHostnames);
      await this._setSyncState({ status: 'syncing', currentPage: 1, totalPages, totalCount });
      ...
      for (let page = 2; page <= totalPages; page++) {
        ...
        // 5페이지마다 상태 업데이트
        if (page % 5 === 0) {
          await this._setSyncState({ status: 'syncing', currentPage: page, totalPages, totalCount });
        }
      }

      // 저장
      await this._persist(allHostnames, totalCount);
      await this._setSyncState({ status: 'complete', lastSync: Date.now(), totalCount: allHostnames.size });

      Logger.info(`[KisaSync] Full sync complete: ${allHostnames.size} hostnames`);
  ...
  async startIncrementalSync(serviceKey) {
    ...
    try {
      await this._setSyncState({ status: 'syncing', type: 'incremental', startedAt: Date.now() });

      // 기존 데이터 로드
      const existing = await chrome.storage.local.get('kisaBlacklist');
      const existingHostnames = new Set(existing.kisaBlacklist?.hostnames || []);
      ...
      await this._setSyncState({ status: 'complete', lastSync: Date.now(), totalCount: existingHostnames.size });

      Logger.info(`[KisaSync] Incremental sync complete: +${newHostnames.size} new, total ${existingHostnames.size}`);
      return { success: true, added: newHostnames.size, total: existingHostnames.size };
    } catch (error) {
